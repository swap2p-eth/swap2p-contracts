...
contract Swap2p {
    ...
    function _removeOfferKey(address m, Side s, FiatCode f) private {
        ...
        if (idx != last) {
            ...
            arr[idx] = lastA;
            _offerPos[lastA][s][f] = pos;
        }
        arr.pop();
        delete _offerPos[m][s][f];
    }
    ...
    function markFiatPaid(uint96 id, string calldata msg_) external {
        Deal storage d = deals[id];
        if (d.state != DealState.ACCEPTED) revert WrongState();
        if ((d.side == Side.BUY && msg.sender != d.maker) || (d.side == Side.SELL && msg.sender != d.taker)) {
            revert NotFiatPayer();
        }
    ...
    function release(uint96 id) external {
        ...
        if ((d.side == Side.BUY && msg.sender != d.taker) || (d.side == Side.SELL && msg.sender != d.maker)) {
            revert NotTaker();
        }
